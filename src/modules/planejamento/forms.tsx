...
export function CommandForm({ title, children, command, submit = 'Salvar', onDone }: { title: string; children: ReactNode; command: (data: FormData) => Command; submit?: string; onDone?: (id: string) => void }) {
  ...
  return <details className="command-box"><summary className="cursor-pointer font-semibold text-blue-900">{title}</summary><form className="mt-4 space-y-4" onSubmit={async e => {
  ...
  }}><fieldset disabled={busy} className="space-y-4">{children}<button className="button" type="submit">{busy ? 'Salvando…' : submit}</button></fieldset>{error && <p className="text-sm text-rose-800" role="alert">{error}</p>}{message && <p className="text-sm text-green-800" role="status">{message}</p>}</form></details>;
}
export function Field({ label, children }: { label: string; children: ReactNode }) { return <label className="block text-sm font-medium text-slate-700"><span className="mb-1 block">{label}</span>{children}</label>; }
...
export function Check({ name, label, defaultChecked = false }: { name: string; label: string; defaultChecked?: boolean }) { return <label className="flex items-center gap-2 text-sm"><input type="checkbox" name={name} defaultChecked={defaultChecked}/>{label}</label>; }
