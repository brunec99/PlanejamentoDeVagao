...
import { LoadState, Missing, Status, Progress, Empty, Callout } from './ui';
export function PlanningOverview({ workId }: { workId: string }) {
  ...
  })}</tbody></table></div>}</section>)}</div><div className="mt-6"><Callout tone="info">O término do takt não conclui o vagão. A terminalidade depende das atividades obrigatórias, dos critérios e dos bloqueios em aberto.</Callout></div></>;
}
