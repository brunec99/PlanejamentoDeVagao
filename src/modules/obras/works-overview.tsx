  ...
  return <>
    ...
    {works.length === 0
      ...
      : <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{works.map(work => {
          ...
          return <Link href={planningPath(work.id)} key={work.id} className="panel group p-5 transition-shadow hover:shadow-md">
            ...
            <span className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-blue-700">Abrir planejamento <ArrowRight size={14} className="transition-transform group-hover:translate-x-0.5" /></span>
          </Link>;
        })}</div>}
  </>;
}
