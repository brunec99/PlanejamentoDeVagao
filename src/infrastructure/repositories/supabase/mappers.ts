...
interface PlanRow { schedule_meta?: {version?: number; calendar?: MediumTermPlan['calendar']; copiedFromPlanId?: string}; id: string; created_at: string; updated_at: string; work_id: string; month: string; name: string; baseline_of: string | null; frozen_at: string | null; created_by: string }
interface PlanTaskRow { schedule_meta?: {version?: number; duration?: PlanTask['duration']; anchorStart?: string; sourceTaskId?: string; actualStart?: string; actualEnd?: string}; id: string; created_at: string; updated_at: string; plan_id: string; name: string; planned_start: string; planned_end: string; team_id: string | null; activity_id: string | null; notes: string | null; progress: number; order_index: number; level: number | null }
interface IfcModelRow { id: string; created_at: string; updated_at: string; work_id: string; name: string; discipline: string }
interface IfcVersionRow { id: string; created_at: string; updated_at: string; model_id: string; version: number; file_name: string; file_size: number; storage_path: string | null; uploaded_by: string; storeys: string[]; element_count: number }
...
export function rowsToPlanningData(rows: Rows): PlanningData {
  return {
    ...
    commitments: rows.weekly_commitments.map((r): WeeklyCommitment => ({ id: r.id, createdAt: r.created_at, updatedAt: r.updated_at, workId: r.work_id, name: r.name, supplier: r.supplier ?? '', activityId: r.activity_id ?? undefined, weekStart: r.week_start, weekEnd: r.week_end, responsibleId: r.responsible_id, teamId: r.team_id ?? undefined, startDate: r.start_date, endDate: r.end_date, fulfilled: r.fulfilled ?? undefined, cause: r.cause ?? undefined, justification: r.justification ?? undefined, recordedAt: r.recorded_at ?? undefined, recordedBy: r.recorded_by ?? undefined })),
    ...
    plans: rows.medium_term_plans.map((r): MediumTermPlan => ({ ...r.schedule_meta, id: r.id, createdAt: r.created_at, updatedAt: r.updated_at, workId: r.work_id, month: r.month, name: r.name, baselineOf: r.baseline_of ?? undefined, frozenAt: r.frozen_at ?? undefined, createdBy: r.created_by })),
    planTasks: rows.plan_tasks.map((r): PlanTask => ({ ...r.schedule_meta, id: r.id, createdAt: r.created_at, updatedAt: r.updated_at, planId: r.plan_id, name: r.name, plannedStart: r.planned_start, plannedEnd: r.planned_end, teamId: r.team_id ?? undefined, activityId: r.activity_id ?? undefined, notes: r.notes ?? undefined, progress: r.progress, order: r.order_index, level: r.level ?? 0 })),
...
export function planningDataToPayload(data: PlanningData) {
  return {
    ...
    weekly_commitments: data.commitments.map(c => ({ id: c.id, created_at: c.createdAt, updated_at: c.updatedAt, work_id: c.workId, name: c.name, supplier: c.supplier, activity_id: c.activityId ?? null, week_start: c.weekStart, week_end: c.weekEnd, responsible_id: c.responsibleId, team_id: c.teamId ?? null, start_date: c.startDate, end_date: c.endDate, fulfilled: c.fulfilled ?? null, cause: c.cause ?? null, justification: c.justification ?? null, recorded_at: c.recordedAt ?? null, recorded_by: c.recordedBy ?? null })),
    ...
    medium_term_plans: data.plans.map(p => ({ schedule_meta: {version:p.version,calendar:p.calendar,copiedFromPlanId:p.copiedFromPlanId}, id: p.id, created_at: p.createdAt, updated_at: p.updatedAt, work_id: p.workId, month: p.month, name: p.name, baseline_of: p.baselineOf ?? null, frozen_at: p.frozenAt ?? null, created_by: p.createdBy })),
    plan_tasks: data.planTasks.map(t => ({ schedule_meta: {version:t.version,duration:t.duration,anchorStart:t.anchorStart,sourceTaskId:t.sourceTaskId,actualStart:t.actualStart,actualEnd:t.actualEnd}, id: t.id, created_at: t.createdAt, updated_at: t.updatedAt, plan_id: t.planId, name: t.name, planned_start: t.plannedStart, planned_end: t.plannedEnd, team_id: t.teamId ?? null, activity_id: t.activityId ?? null, notes: t.notes ?? null, progress: t.progress, order_index: t.order, level: t.level })),
