...
interface PlanRow { schedule_meta?: {version?: number; calendar?: MediumTermPlan['calendar']}; id: string; created_at: string; updated_at: string; work_id: string; month: string; name: string; baseline_of: string | null; frozen_at: string | null; created_by: string }
interface PlanTaskRow { schedule_meta?: {version?: number; duration?: PlanTask['duration']; anchorStart?: string; sourceTaskId?: string}; id: string; created_at: string; updated_at: string; plan_id: string; name: string; planned_start: string; planned_end: string; team_id: string | null; activity_id: string | null; notes: string | null; progress: number; order_index: number; level: number | null }
...
export function planningDataToPayload(data: PlanningData) {
  return {
    ...
    medium_term_plans: data.plans.map(p => ({ schedule_meta: {version:p.version,calendar:p.calendar}, id: p.id, created_at: p.createdAt, updated_at: p.updatedAt, work_id: p.workId, month: p.month, name: p.name, baseline_of: p.baselineOf ?? null, frozen_at: p.frozenAt ?? null, created_by: p.createdBy })),
