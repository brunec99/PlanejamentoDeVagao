...
As predecessoras seguem a sintaxe do Project — `12`, `12II+2d`, `12TT-1d`, `12TI+2dd`, separadas por vírgula — nos quatro tipos de vínculo (TI, II, TT, IT), com defasagem em dias úteis (`d`) ou corridos (`dd`). O número é a **posição na lista**, não o número hierárquico do item: o hierárquico muda a cada recuo e a referência passaria a apontar outra linha. Ter só Término-Início descrevia mal a obra — a alvenaria do 5º não espera a do 4º terminar, ela começa alguns dias depois de a outra começar, que é Início-Início com defasagem.

As datas não se movem sozinhas, por decisão de escopo: a rede aponta a incoerência, dizendo qual ponta o vínculo prende e qual a data mais cedo que ele permite, e a reprogramação é manual. Ligações que fechariam um ciclo são recusadas.

A grade abre **bloqueada**, com cadeado para liberar — em reunião, o risco não é a edição que falta, é a acidental. Item de resumo recolhe e expande, o teclado segue o padrão do Project (F2, Esc, Enter desce, Tab recua) e as colunas calculadas mostram % alvo, desvio em pontos percentuais e, com linha de base escolhida, término da base e variação em dias. O % alvo é leitura de **tempo decorrido**, não medição física, e a tela diz isso: confundir os dois vira um avanço que ninguém mediu.
