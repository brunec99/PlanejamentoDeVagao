...
**Médio prazo** (`/obras/{obraId}/medio-prazo`, aba 3): abaixo da grade ficam a **alocação de recursos semana a semana**, com superalocação por equipe, e a **linha de base e atrasos**, com a variação de início e término em dias úteis — detalhes em [Planejamento em três níveis](docs/planejamento-tres-niveis.md). O **cronograma do mês** segue o layout do MS Project que a equipe usa: tabela à esquerda e Gantt de acompanhamento à direita, na mesma linha e com a mesma rolagem. As colunas são Id, Indicadores, % concluída, Nome da tarefa, Duração, Início real, Término real, Início, Término, Início e Término da linha de base, Predecessoras e Nomes dos recursos. As datas aparecem como "Seg 24/08/26", e a linha 0 resume o cronograma inteiro. Há **um cronograma por mês, com horizonte de três meses**. O primeiro nasce em branco, e cada mês seguinte nasce como **cópia do anterior**, que fica guardado como estava. Detalhes em [Planejamento em três níveis](docs/planejamento-tres-niveis.md), etapa 4.
...
Um item com subitens é **resumo**, e não tem datas próprias: início, término e avanço dele são os dos subitens, calculados na leitura por `rollUpPlan` (`src/domain/rules.ts`) — datas pelo envelope, percentual pela média ponderada pela duração. Guardar esses valores seria manter duas verdades sobre a mesma data: quem edita o subitem esperaria o item acompanhar, e ele não acompanharia.
...
**A linha de base é o próprio plano congelado**: "Definir linha de base" duplica o plano com `frozenAt` marcado, e o congelado não aceita edição. A mais recente aparece por padrão nas colunas da base e como barra cinza sob a barra azul do Gantt. As linhas são pareadas por `sourceTaskId`, que a cópia congelada guarda.
...
As predecessoras seguem a sintaxe do Project — `12`, `12;14`, `27TI+6 dias`, `12II+2 dias corridos`, `5TT-1 dia` (também aceitam `2d`/`2dd` e vírgula) — nos quatro tipos de vínculo (TI, II, TT, IT), com defasagem em dias úteis (`d`) ou corridos (`dd`). O número é o **Id** (a posição na lista), não o número hierárquico do item: o hierárquico muda a cada recuo e a referência passaria a apontar outra linha. Ter só Término-Início descrevia mal a obra — a alvenaria do 5º não espera a do 4º terminar, ela começa alguns dias depois de a outra começar, que é Início-Início com defasagem.

As datas são **agendadas automaticamente**, como no Project: mudar a duração ou a predecessora empurra as sucessoras no rascunho, e o servidor refaz o cálculo ao gravar. Um início digitado numa tarefa com predecessoras vale como "Não iniciar antes de", e apagar o início devolve a data à rede. Marco é a tarefa de 0 dias. Lançar % concluída preenche o Início real, e 100% preenche o Término real. A data real prende a tarefa. Ligações que fechariam um ciclo são recusadas. Nada vai ao banco até "Revisar e salvar", que pede motivo.

A grade abre **bloqueada**, com cadeado para liberar — em reunião, o risco não é a edição que falta, é a acidental. Item de resumo recolhe e expande, o teclado segue o padrão do Project (F2, Esc, Enter desce, Insert insere acima, Alt+Shift+→ recua, Ctrl+F2 vincula) e as colunas calculadas mostram % alvo, desvio em pontos percentuais e, com linha de base escolhida, término da base e variação em dias. O % alvo é leitura de **tempo decorrido**, não medição física, e a tela diz isso: confundir os dois vira um avanço que ninguém mediu.

**Curto prazo** (`/obras/{obraId}/curto-prazo`): a planilha de produção que a equipe preenche toda semana, com a mesma estrutura do Sheets que ela substitui — fornecedor, semana, início, término, atividade, equipe, o calendário de segunda a sábado, e no fechamento o Status (Sim/Não), a causa e a justificativa.

**Ela não depende de mais nada do sistema.** Fornecedor e atividade são texto escrito na própria linha, a equipe é opcional e o vínculo com uma atividade do cronograma é opcional — a planilha real mistura frentes de obra com tarefas que não existem em cronograma algum ("Diário de obra", "GFIP", "Visita", "NF - ..."). A mesma linha pode repetir na semana. Antes disso, escrever uma linha exigia uma equipe cadastrada em outra tela, e a empresa vinha do cadastro dela: quem preenche a semana ficava dependente de um cadastro que na obra ninguém faz antes.

A planilha imita o Google Sheets que a equipe usa. Empresa, Semana, Equipe, Status e Causas são **listas suspensas em pílula**, e Status fica verde em Sim e vermelho em Não. Todo cabeçalho tem o **funil de filtro** do Sheets, com classificação A→Z/Z→A e marcação de valores. A ordem padrão agrupa por empresa e, dentro dela, por início. A lista de Empresa junta o cadastro da obra e o que já foi digitado, e aceita nome novo. A lista de Equipe mostra só as equipes da empresa da linha. Uma equipe digitada que ainda não existe entra no cadastro daquela empresa com capacidade 3, ajustável em Configurações. A lista de Semana vai da primeira semana da obra até 4 semanas à frente. **A causa só é pedida quando o Status é Não, e é obrigatória**: escolher Não deixa a linha marcada até a causa ser escolhida, e só então o apontamento é gravado. Antes, a tela gravava a primeira causa da lista sozinha, o que punha causa falsa no Pareto. Os componentes estão em `src/modules/curto-prazo/sheet-controls.tsx`, e o filtro e a ordem em `sheet-view.ts`.

A única automação é o **calendário**: as células de segunda a sábado se preenchem a partir de início e término, que é o que o engenheiro digita. Já foi o contrário — os dias eram marcados e as datas calculadas — e por isso a coluna `weekdays` deixou de existir: uma coluna que ninguém mais escreve não fica no banco fingindo que ainda decide algo. Todo campo da linha é editável, inclusive a semana; mover a linha de semana desloca o período com ela.
...
**Seccionamento**: os visualizadores de Modelos IFC e Modelo federado oferecem corte horizontal (Y) ou vertical (X/Z), posição de 0 a 100% por controle deslizante ou campo numérico e inversão do lado visível. Na federação, um único plano atravessa todos os modelos carregados. O corte combina com os filtros de pavimento/visibilidade e pode ser removido; é temporário, reinicia ao carregar outra geometria e não altera arquivos, quantitativos nem composições salvas. A busca de elementos continua abrangendo os dois lados do plano. Não exige migração; o BIM 4D permanece com seus controles existentes.
...
- `/obras/obra-1/longo-prazo`: Planejador de longo prazo (fluxograma → Linha de Balanço); `?visao=prevision` mostra a Linha de Balanço das atividades dos vagões, restrições e linhas de base.
